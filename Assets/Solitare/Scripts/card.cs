...
public class card : MonoBehaviour {
    ...
    void Generate()
    {
        ...
        if (card_suit_selected == card_suit.random)
            my_suit = Util.GetRandomBetween0And(3);

        if (card_rank_selected == card_rank.random)
        {
            bool approved_rank = false;
            my_rank = Util.GetRandomBetween0And(12);
            ...
            while (!approved_rank)
            {
				...
				if (deck.GetInstance().card_on_board_by_rank[my_rank] <= deck.GetInstance().max_number_of_card_with_the_same_rank_on_board)//if you don't have use this rank too much on the board
                {
			...
			deck.GetInstance().card_on_board_by_rank[my_rank]++;
        }
    ...
    public bool TurnThisCard()
    {
        ...
        //Debug.Log(my_suit + " " + my_rank + " " + face_up);
    }
