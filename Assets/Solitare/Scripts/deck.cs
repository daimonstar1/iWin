...
//it is use to show an animation when a card pass from board to target deck or conversely
  ...
  public IEnumerator MoveTo (GameObject go, Transform start_point, Transform end_point, bool refresh_board)//move the ghost card
  {
    ...
    while (time < 1) {
      ...
      /*Vector3 destination = new Vector3(angle,ghost_card.transform.rotation.eulerAngles.y,ghost_card.transform.rotation.eulerAngles.z);
			Vector3 nrot = Vector3.Lerp(ghost_card.transform.rotation.eulerAngles, 
				destination, 
				deltaT);
			ghost_card.transform.rotation.eulerAngles.Set (nrot.x, nrot.y, nrot.z);*/
