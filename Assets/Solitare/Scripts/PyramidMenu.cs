...
public class PyramidMenu : DefaultMenu
{
  ...
  private CanvasGroup pauseDialog_cg;
  ...
  public Animator combo_moveAnimator;
  public GameObject combo_score_event;
  ...
  public static PyramidMenu instance = null;
  //this script
  ...
  void Awake ()
  {
    ...
    GameObject combo = MenuManager.Find (gameObject, "ComboInfo_Image", true);
    combo_moveAnimator = combo.GetComponent<Animator> ();
    ...
    combo_score_event = MenuManager.Find (gameObject, "ComboScoreEvent", true);
  }

  public static PyramidMenu GetInstance ()
  {
    return instance;
  }
  ...
  public void ClosePopups ()
  {
    ...
    //deck.GetInstance().enableCards(true);
    deck.GetInstance ().CheckPlayerCanMove ();
    ...
    endDialog_cg.interactable = false;
    if (endDialog_moveAnimator.gameObject.active)
  ...
  public void OpenBlackFade ()
  {
    ...
    deck.GetInstance ().player_can_move = false;
  ...
  public void OpenPopup (int p)
  {
    if (p == 0 || p == 1 || p == 2 || p == 5 || p == 6) {
      ...
      //deck.this_deck.enableCards(false);
      deck.GetInstance ().player_can_move = false;
    }
    ...
    case 3:
      {
        MenuManager.GetInstance ().PlaySound (MenuManager.SoundNamesEnum.DIALOG_SOUND);
        combo_moveAnimator.SetTrigger ("InState");
        break;
      }
    // Tip
    case 4:
      {
        ...
        deck.GetInstance ().player_can_move = false;
        deck.GetInstance ().timer_paused = true;
        panel_cg.interactable = true;
        panel_cg.blocksRaycasts = true;
        break;
      }
  ...
  public override void OnButtonClick (Button bt)
  {
    ...
    case "CloseTuto_Button":
      {
        MenuManager.GetInstance ().PlaySound (MenuManager.SoundNamesEnum.BUTTON_SOUND);
        ClosePopups ();
        StartCoroutine (deck.GetInstance ().StartGame (tutorialDialog_moveAnimator.GetCurrentAnimatorClipInfo (0).Length - 0.1f));
        break;
      }
    case "DontShowAgain_Button":
      {
        MenuManager.GetInstance ().PlaySound (MenuManager.SoundNamesEnum.BUTTON_SOUND);
        PlayerData.GetInstance ().SetValue ("pyramidHTP", "1");
    ...
    case "CloseTip_Button":
      {
        MenuManager.GetInstance ().PlaySound (MenuManager.SoundNamesEnum.BUTTON_SOUND);
        PlayerData.GetInstance ().SetValue ("pyramidTips0", "1");
        PlayerData.GetInstance ().SavePlayerData ();
        deck.GetInstance ().timer_paused = false;
        ClosePopups ();
        break;
      }
    ...
    case "Confirmation_Button":
      {
        MenuManager.GetInstance ().PlaySound (MenuManager.SoundNamesEnum.BUTTON_SOUND);
        StartCoroutine (deck.GetInstance ().GameOver (2));
        break;
      }
  ...
  {
    if (pauseStatus && pauseDialog_cg.interactable == false && tutorialDialog_cg.interactable == false && !deck.GetInstance ().game_end) {
      deck.GetInstance ().timer_paused = true;
      OpenPopup (5);
    }
  }
